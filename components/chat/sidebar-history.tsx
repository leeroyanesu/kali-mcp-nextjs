...
export function SidebarHistory({
  ...
  const localChats = useLiveQuery(
    () => dexieDb.chats.orderBy("createdAt").reverse().toArray(),
    []
  );

  const isLoading = isServerLoading && localChats === undefined;
  ...
  const hasEmptyChatHistory = 
    (!paginatedChatHistories || paginatedChatHistories.every((page) => page.chats.length === 0)) &&
  ...
  const handleDelete = () => {
    ...
    // Delete from Dexie
    dexieDb.transaction("rw", dexieDb.chats, dexieDb.messages, async () => {
      await dexieDb.messages.where("chatId").equals(chatToDelete!).delete();
      await dexieDb.chats.delete(chatToDelete!);
    }).catch(err => console.error("Failed to delete local chat:", err));
    ...
    <>
      <SidebarGroup className="group-data-[collapsible=icon]:hidden">
        ...
        <SidebarGroupContent>
          <SidebarMenu>
            {(() => {
                const serverChats = paginatedChatHistories?.flatMap(
                  (paginatedChatHistory) => paginatedChatHistory.chats
                ) ?? [];

                // Deduplicate chats by ID, preferring server version if available
